...
async function testStock(symbol) {
  try {
     ...
     console.log('Quote for', symbol, ':', quote.priceInfo?.lastPrice || 'fail');

     const chartUrl = 'https://www.nseindia.com/api/chart-databyindex?index=' + encodeURIComponent(symbol) + '&indices=false';
     const cRes = await fetch(chartUrl, { headers: { ...NSE_HEADERS, Cookie: cookies } });
     const chart = await cRes.json();
     console.log('Chart for', symbol, ':', chart.grapthData?.length || 'fail');
  } catch (e) {
